...
async function updateFandomImages() {
  try {
    ...
    // Check if these images are already in
    const newImages = [
    ...
    const existingImages = new Set(data.filter(m => m.image).map(m => {
      // Decode the URL in case it's url-encoded in the database
    ...
    for (const img of newImages) {
      if (!existingImages.has(img)) {
        data.push({
          id: ++maxId,
          title: "Fandom Feature " + (added + 1),
          type: "Movie",
          genre: "Drama",
          desc: "Newly added content highlighting our Fandom collection.",
          image: "/images/fandom/" + encodeURIComponent(img),
          tags: ["fandom"] // This is what adds it to the Fandom page
        });
        added++;
